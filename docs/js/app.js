...
const source=[...(tab==='drivers'?s.drivers:s.constructors)].sort((a,b)=>{if(b.total!==a.total)return b.total-a.total;const pa=(POSITIONS[currentYear]||{})[a.id]||[],pb=(POSITIONS[currentYear]||{})[b.id]||[];for(let pos=1;pos<=20;pos++){const ca=pa.filter(r=>r===String(pos)).length,cb=pb.filter(r=>r===String(pos)).length;if(cb!==ca)return cb-ca;}return 0;});
